...
(function () {
          ...
          '<div class="card" style="margin-top:14px;border-color:var(--gold);background:var(--gold-soft)">' +
            ...
            '<div style="margin-bottom:8px">' +
              students.filter(function (s) { return !hasReports[s.id]; }).map(function (s) {
                return '<a href="#/student/' + s.id + '?date=' + today + '" style="color:var(--link);text-decoration:underline;margin-right:10px;font-size:.85rem">' + esc(s.name) + '</a>';
              }).join('') +
            '</div>' +
    ...
    app.innerHTML = '' +
      ...
      '<main class="app-main">' +
        ...
        '<div class="field">' +
          ...
          '</select>' +
        '</div>' +
        '<div class="section-title">' + esc(cls.name) + '</div>' +
        (students.length === 0 ?
          ...
          students.map(function (s) {
            ...
            return (
              '<div class="student-row fee-row" data-sid="' + s.id + '">' +
                '<div>' +
                  '<div style="font-weight:600"><button class="linky" data-action="fee-history" data-sid="' + s.id + '">' + esc(s.name) + '</button></div>' +
                '</div>' +
      ...
      '<main class="app-main">' +
        ...
        (students.length ? '<div class="card" style="margin-top:14px;padding:14px">' +
          ...
          '<div class="stat-row">' +
            ...
            '<div class="stat"><div class="n" style="color:var(--ok)">' + num(totalPaid) + '</div><div class="l">' + t('collected') + '</div></div>' +
            '<div class="stat"><div class="n" style="color:' + (totalOut > 0 ? 'var(--bad)' : 'var(--ok)') + '">' + num(totalOut) + '</div><div class="l">' + t('outstanding') + '</div></div>' +
          '</div>' +
          ...
          '<div class="tbl-wrap"><table class="tbl">' +
            ...
            '<tbody>' +
              students.map(function (s) {
                ...
                return (
                  '<tr data-sid="' + s.id + '">' +
                    '<td><button class="linky" data-action="p-fee-history" data-sid="' + s.id + '">' + esc(s.name) + '</button></td>' +
  ...
  async function openFeeHistory(session, sid, isPrincipal) {
    ...
    const rows = months.map(function (mo) {
      ...
      const paid = ps && ps.paid;
      const who = ps && ps.markedBy ? (users[ps.markedBy] ? users[ps.markedBy].name : ps.markedBy) : '—';
    ...
    b.innerHTML =
      '<div class="modal modal-wide">' +
        '<h3>' + esc(student.name) + ' — ' + t('feeHistory') + '</h3>' +
  ...
  async function renderReports(session) {
    ...
    if (session.role === 'qari') {
      ...
      title = (users[session.id] ? users[session.id].name : t('reports'));
    } else {
    ...
    for (const s of students) {
      ...
      gridRows.push(
        '<div class="class-att-row">' +
          '<div class="ca-name"><span style="font-weight:600">' + esc(s.name) + '</span>' +
  ...
  async function renderQuickEntry(session) {
          ...
          return (
            '<div class="card" style="margin-top:12px">' +
              '<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap">' +
                ...
                '</div>' +
                '<div class="seg">' +
                  '<button type="button" class="opt present' + (st.present ? ' on present' : '') + '" data-action="qe-present" data-sid="' + s.id + '" data-v="1">' + t('present') + '</button>' +
                  '<button type="button" class="opt absent' + (!st.present ? ' on absent' : '') + '" data-action="qe-present" data-sid="' + s.id + '" data-v="0">' + t('absent') + '</button>' +
                '</div>' +
              '</div>' +
              (st.present ?
                '<div style="margin-top:10px">' +
                  '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">' +
                    '<div class="field"><label for="qe-pages-' + s.id + '">' + t('sabaq') + ' — ' + t('pages') + '</label>' +
                      '<select id="qe-pages-' + s.id + '">' + rangeOpts(10, st.pages) + '</select></div>' +
                    '<div class="field"><label for="qe-lines-' + s.id + '">' + t('lines') + '</label>' +
                      '<select id="qe-lines-' + s.id + '">' + rangeOpts(20, st.lines) + '</select></div>' +
                  '</div>' +
                  '<div style="display:flex;gap:10px;margin-top:10px;flex-wrap:wrap">' +
                    '<label class="switch"><input type="checkbox" id="qe-sabqi-' + s.id + '"' + (st.sabqiDone ? ' checked' : '') + '><span class="track"></span><span class="lbl" style="margin-left:6px">' + t('sabqi') + '</span></label>' +
                    '<label class="switch"><input type="checkbox" id="qe-manzil-' + s.id + '"' + (st.manzilDone ? ' checked' : '') + '><span class="track"></span><span class="lbl" style="margin-left:6px">' + t('manzil') + '</span></label>' +
                  '</div>' +
                  (st.manzilDone ?
                  ...
                  '<div class="field" style="margin-top:8px">' +
                    '<input id="qe-comment-' + s.id + '" placeholder="' + t('commentHint') + '" value="' + esc(st.comment) + '">' +
                  '</div>' +
                '</div>' : '') +
            '</div>'
          );
    ...
    viewActions['qe-save-all'] = function () {
      ...
      const jobs = students.map(function (s) {
        const st = state[s.id];
        st.pages = parseInt((document.getElementById('qe-pages-' + s.id) || { value: 0 }).value, 10) || 0;
        st.lines = parseInt((document.getElementById('qe-lines-' + s.id) || { value: 0 }).value, 10) || 0;
        const sabaqBox = document.getElementById('qe-sabqi-' + s.id);
        const manzilBox = document.getElementById('qe-manzil-' + s.id);
        st.sabqiDone = !!(sabaqBox && sabaqBox.checked);
        st.manzilDone = !!(manzilBox && manzilBox.checked);
        const cm = document.getElementById('qe-comment-' + s.id);
        if (cm) st.comment = cm.value;
        const rep = {
          present: st.present,
          sabaqDone: st.present && st.pages + st.lines > 0,
          pages: st.present && st.pages > 0 ? st.pages : null,
          lines: st.present && st.lines > 0 ? st.lines : null,
          sabqiDone: st.present && st.sabqiDone,
          manzilDone: st.present && st.manzilDone,
          manzil: st.present && st.manzilDone ? st.manzil : null,
          comment: st.present && st.comment.trim() ? st.comment.trim() : null
        };
    ...
    const cards = students.map(function (s) {
      ...
      return (
        '<div class="student-row" style="display:block;padding:14px">' +
          '<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap">' +
            '<div>' +
              '<div style="font-weight:700">' + esc(s.name) + ' <span class="badge badge-cat">' + esc(s.category) + '</span></div>' +
              '<div class="meta">' + t('para') + ' ' + num(s.para) + ' · ' + t('page') + ' ' + num(s.currentPage || '—') + '</div>' +
            '</div>' +
  ...
  async function exportStudentReportCard(sid) {
    ...
    const w = window.open('', '_blank');
    w.document.write('<!DOCTYPE html><html><head><title>' + esc(st.name) + ' — Report Card</title><style>' +
      ...
      '<div class="head"><div>' +
        ...
        '<div class="sub">Report Card — ' + esc(cls.name) + '</div>' +
      '</div><div class="right">Generated: ' + new Date().toLocaleDateString() + '</div></div>' +

      '<h2>Student</h2>' +
      '<div class="grid">' +
        '<div class="g"><b>' + esc(st.name) + '</b>' + 'Student Name</div>' +
