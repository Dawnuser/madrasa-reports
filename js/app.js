...
(function () {
  ...
  /* two-letter initials: Ustad/Sheikh is a title prefix, the name starts after
     it; hyphenated parts count (Atta-ul-Rahman → AR, Anees → A) */
  function initialsOf(name) {
    const parts = String(name || '').replace(/^(sheikh|ustad)\s+/i, '').trim().split(/[\s\-]+/).filter(Boolean);
  ...
  async function route() {
    ...
    try {
      ...
      else if (seg[0] === 'huffaz') await renderHuffaz(session);
        ...
        '<div class="p-card-top">' +
          '<div>' +
            ...
            '<div class="meta">' + t('parentClass') + ': ' + nameDisplay(cls ? cls.name : '—') + ' · ' + (isQaida ? t('lessonNo') : t('para')) + ' ' + num(st.para) +
              (st.currentPage && !isQaida ? ' · ' + t('page') + ' ' + num(st.currentPage) : '') +
              (!isQaida && (st.fromStart != null || st.fromEnd != null) ? ' · ' + t('fromStart') + ' ' + num(st.fromStart || 0) + ' · ' + t('fromEnd') + ' ' + num(st.fromEnd || 0) : '') +
              (st.testsPassed > 0 ? ' · ✓ ' + t('tests') + ' ' + num(st.testsPassed) : '') + '</div>' +
          '</div>' +
...
const manzilIsTri = parentTrack(st, classes) === 'hifz';
  ...
  function captureWidgets() {
    ...
    if (ln) draft.lines = parseFloat(ln.value) || 0; /* parseFloat: qaida lines include 0.5 */
    ...
    const fmp2 = g('f-manzil-para');
    if (fmp2) draft.manzilPara = parseInt(fmp2.value, 10) || 0;
  ...
  async function renderStudentForm(session, sid, ds) {
    ...
    app.innerHTML = '' +
      ...
      '<main class="app-main">' +
        ...
        '<div class="pill" style="margin-top:8px">' + (isQaida ? t('lessonNo') : t('para')) + ' ' + num(student.para) +
          ...
          (track !== 'qaida' && (student.fromStart != null || student.fromEnd != null) ? ' · ' + t('fromStart') + ' ' + num(student.fromStart || 0) + ' · ' + t('fromEnd') + ' ' + num(student.fromEnd || 0) : '') +
          (track === 'hifz' && student.testsPassed > 0 ? ' · ✓ ' + t('tests') + ' ' + num(student.testsPassed) : '') +
        ...
        '<div class="card" style="margin-top:14px">' +
          ...
          (rep.present ?
            ...
            '<div class="reveal' + (rep.sabaqDone ? ' open' : '') + '" id="reveal-sabaq">' +
              '<div class="sub-row">' +
                ...
                '<div class="field"><label for="f-lines">' + t('lines') + '</label>' +
                  '<select id="f-lines"' + (locked ? ' disabled' : '') + '>' + (isQaida ? linesOpts(rep.lines) : rangeOpts(20, rep.lines)) + '</select></div>' +
              '</div>' +
            '</div>' +
            (showSabqi ?
              '<div class="tick-row">' +
            ...
            (showManzil && manzilIsTri ?
              '<div class="reveal' + (rep.manzilDone ? ' open' : '') + '" id="reveal-manzil">' +
                '<div class="sub-row">' +
                  '<div class="field"><label for="f-manzil-para">' + t('manzilPara') + '</label>' +
                    '<select id="f-manzil-para"' + (locked ? ' disabled' : '') + '>' + rangeOpts(30, rep.manzilPara) + '</select></div>' +
                  '<div class="field"><label for="f-manzil">' + t('manzil') + '</label>' +
          ...
          (!rep.present ?
            '<div class="card" style="margin-top:14px">' +
            '<div class="tick-row">' +
              '<div><div class="lbl">' + t('reason') + '</div><div class="sub">' + t('reasonSub') + '</div></div>' +
              '<label class="switch"><input type="checkbox" id="sw-reason"' + (rep.reason ? ' checked' : '') + (locked ? ' disabled' : '') + '><span class="track"></span></label>' +
            '</div>' +
            '<div class="field" style="margin-top:12px">' +
              '<label for="f-comment">' + t('commentForParents') + ' <small style="color:var(--ink-soft)">' + t('commentHint') + '</small></label>' +
              '<textarea id="f-comment" rows="2" placeholder="' + t('commentHint') + '"' + (locked ? ' disabled' : '') + '>' + esc(rep.comment || '') + '</textarea>' +
    ...
    viewActions['save-report'] = function () {
      ...
      if (rep2.manzilDone && manzilIsTri && !rep2.manzilPara) {
        toast(t('manzilParaRequired'));
        return;
      }
  ...
  /* qaida lines: —, 0.5 (half line), 1–20. 0.5 needs the
     reports.lines numeric migration (see migration-qaida-half-line.sql). */
  function linesOpts(selected) {
    let out = '<option value="0">—</option>';
    const sel = Number(selected) || 0;
    const vals = [0.5];
    for (let i = 1; i <= 20; i++) vals.push(i);
    vals.forEach(function (v) {
      out += '<option value="' + v + '"' + (sel === v ? ' selected' : '') + '>' + num(v) + '</option>';
    });
    return out;
  }
      ...
      '<main class="app-main">' +
        ...
        '<div style="display:grid;gap:10px;margin-top:18px">' +
          ...
          '<a class="class-row" href="#/pfees">' +
            ...
            '<span style="margin-inline-start:auto;color:var(--gold)">&rsaquo;</span>' +
          '</a>' +
          '<a class="class-row" href="#/huffaz">' +
            '<span class="avatar">🎓</span>' +
            '<span><span class="nm">' + t('huffaz') + '</span><br><span class="sub">' + t('huffazSub') + '</span></span>' +
            '<span style="margin-inline-start:auto;color:var(--gold)">&rsaquo;</span>' +
          '</a>' +
  ...
  /* ---------- HUFFAZ (principal only, graduated Hafiz students) ---------- */
  async function renderHuffaz(session) {
    const t = I18N.t;
    if (session.role !== 'principal') { redirect('dashboard'); return; }
    ...
    app.innerHTML = '' +
      topbar(true) +
      '<main class="app-main">' +
        '<span class="eyebrow">' + t('huffaz') + '</span>' +
        '<h1 class="h-display" style="font-size:1.3rem">🎓 ' + esc(t('huffaz')) + '</h1>' +
        '<p style="color:var(--ink-soft);font-size:.88rem;margin-top:6px">' + t('huffazSub') + '</p>' +
        '<div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap;align-items:center" class="no-print">' +
          '<button class="btn btn-primary btn-sm" data-action="add-hafiz">+ ' + t('addHafiz') + '</button>' +
          exportGroup([
            { action: 'export-huffaz', label: '⬇ ' + t('exportExcel') }
          ]) +
        '</div>' +
        '<div class="section-title">' + t('huffaz') + ' (' + num(list.length) + ')</div>' +
        (list.length === 0 ?
          '<div class="empty-note">' + t('noHuffaz') + '</div>' :
          list.map(function (h) {
            return (
              '<div class="student-row">' +
                ...
                '<div class="row-actions">' +
                  ...
                  '<button class="icon-mini" data-action="edit-hafiz" data-id="' + h.id + '" title="' + t('edit') + '">✎</button>' +
                  '<button class="icon-mini danger" data-action="del-hafiz" data-id="' + h.id + '" title="' + t('remove') + '">🗑</button>' +
                '</div>' +
              '</div>'
            );
          }).join('')) +
    ...
    const openHafizModal = function (h) {
      const isEdit = !!h.id;
      const b = document.createElement('div');
      b.className = 'modal-back';
      b.innerHTML =
      ...
      b.addEventListener('click', function (e) {
        const a = e.target.closest('[data-action]');
        if (!a) return;
        if (a.getAttribute('data-action') === 'h-cancel' || e.target === b) {
          b.remove();
          return;
        }
        if (a.getAttribute('data-action') === 'h-save') {
          const name = document.getElementById('h-name').value.trim();
          if (!name) { toast(t('nameRequired')); return; }
          const yrRaw = document.getElementById('h-year').value;
          DB.saveHafiz({
            id: h.id || null,
            name: name,
            graduationYear: yrRaw === '' ? null : (parseInt(yrRaw, 10) || null),
            parentName: document.getElementById('h-pname').value.trim(),
            parentNumber: document.getElementById('h-pphone').value.replace(/[^\d+]/g, ''),
            completedUnder: h.completedUnder || '',
            notes: document.getElementById('h-notes').value.trim()
          }).then(function () {
            b.remove();
            renderHuffaz(session);
          });
        }
      });
    };

    const openHafizView = function (h) {
      const b = document.createElement('div');
      b.className = 'modal-back';
      const row = function (label, val) {
        return '<div class="field"><label>' + label + '</label><div class="meta" style="font-size:1rem;color:var(--ink)">' + (val ? esc(val) : '—') + '</div></div>';
      };
      b.innerHTML =
        '<div class="modal">' +
      ...
      b.addEventListener('click', function (e) {
        const a = e.target.closest('[data-action]');
        if ((a && a.getAttribute('data-action') === 'h-close') || e.target === b) b.remove();
      });
    };

    viewActions['add-hafiz'] = function () { openHafizModal({}); };
    viewActions['edit-hafiz'] = function (btn) {
      const h = list.find(function (x) { return x.id === btn.getAttribute('data-id'); });
      if (h) openHafizModal(h);
    };
    viewActions['view-hafiz'] = function (btn) {
      const h = list.find(function (x) { return x.id === btn.getAttribute('data-id'); });
      if (h) openHafizView(h);
    };
    viewActions['del-hafiz'] = function (btn) {
      const h = list.find(function (x) { return x.id === btn.getAttribute('data-id'); });
      if (!h) return;
      if (!confirm(t('confirmFirstDelete') + '\n\n' + nameClean(h.name))) return;
      if (!confirm(t('confirmSecondDelete'))) return;
  ...
  async function renderClass(session, cid) {
    ...
    function renderStudentGroups(list) {
      ...
      shifts.forEach(function (sh) {
        ...
        out += '<div class="shift-group">' +
               '<div class="shift-header" data-action="toggle-shift" data-shift="' + sh.key + '" style="cursor:pointer"><span class="shift-arrow">' + (collapsedShifts[sh.key] ? '▸' : '▾') + '</span> 🕗 ' + sh.label + ' <span class="badge">' + grp.length + ' ' + t('shiftStudents') + '</span></div>' +
      ...
      if (unassigned.length) {
        out += '<div class="shift-group">' +
               '<div class="shift-header" data-action="toggle-shift" data-shift="un" style="cursor:pointer"><span class="shift-arrow">' + (collapsedShifts['un'] ? '▸' : '▾') + '</span> — ' + t('shiftNone') + ' <span class="badge">' + unassigned.length + ' ' + t('shiftStudents') + '</span></div>' +
    ...
    viewActions['toggle-shift'] = function (btn) {
      const key = btn.getAttribute('data-shift');
      collapsedShifts[key] = !collapsedShifts[key];
      const body = document.getElementById('shift-body-' + key);
      if (body) body.style.display = collapsedShifts[key] ? 'none' : '';
      const arrow = btn.querySelector('.shift-arrow');
      if (arrow) arrow.textContent = collapsedShifts[key] ? '▸' : '▾';
    };
        ...
        '<div class="student-row">' +
          '<div>' +
            '<div style="font-weight:600">' + nameDisplay(s.name) + (ty === 'hifz' ? ' <span class="badge badge-cat">' + esc(s.category) + '</span> ' : '') +
              (ty === 'hifz' && s.testsPassed > 0 ? '<span class="badge badge-ok">✓ ' + t('tests') + ' ' + num(s.testsPassed) + '</span> ' : '') +
              (ty === 'hifz'
            ...
            '<div class="meta">' + (ty === 'qaida' ? t('lessonNo') + ' ' + num(s.para) : t('para') + ' ' + num(s.para) + ' · ' + t('page') + ' ' + num(s.currentPage || '—')) + ' · ' + t('age') + ' ' + num(s.age) + ((ty !== 'qaida' && (s.fromStart != null || s.fromEnd != null)) ? ' · ' + t('fromStart') + ' ' + num(s.fromStart || 0) + ' · ' + t('fromEnd') + ' ' + num(s.fromEnd || 0) : '') + ' · ' + esc(s.parentName) + ' · ' + esc(s.parentNumber) + '</div>' +
          '</div>' +
          '<div class="row-actions">' +
    ...
    const openModal = async function (st) {
      ...
      const applyTrackUi = function () {
        ...
        document.getElementById('m-tests-wrap').style.display = isHifz ? '' : 'none';
        document.getElementById('m-from-wrap').style.display = isQaida ? 'none' : '';
      ...
      b.innerHTML =
        '<div class="modal">' +
          ...
          '</div>' +
          '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px" id="m-from-wrap">' +
            '<div class="field"><label>' + t('fromStart') + '</label><input id="m-fromstart" type="number" min="0" max="30" value="' + (st.fromStart != null ? st.fromStart : '') + '"></div>' +
            '<div class="field"><label>' + t('fromEnd') + '</label><input id="m-fromend" type="number" min="0" max="30" value="' + (st.fromEnd != null ? st.fromEnd : '') + '"></div>' +
          '</div>' +
          ...
          '<div class="field" id="m-tests-wrap"><label>' + t('tests') + ' — <small style="color:var(--ink-soft)">' + t('testsHint') + '</small></label>' +
            '<div style="display:flex;gap:8px;flex-wrap:wrap">' +
              [5, 10, 15, 20, 25, 30].map(function (ms) {
                return '<label style="display:inline-flex;align-items:center;gap:6px;border:1px solid var(--line);border-radius:8px;padding:8px 12px;cursor:pointer">' +
                  '<input type="checkbox" id="m-test-' + ms + '"' + (testsCur >= ms ? ' checked' : '') + '> ' + num(ms) +
                '</label>';
              }).join('') +
            '</div></div>' +
          '<div style="display:flex;gap:10px">' +
      ...
      b.addEventListener('change', function (e) {
        ...
        if (e.target && e.target.id && e.target.id.indexOf('m-test-') === 0) {
          const mv = parseInt(e.target.id.replace('m-test-', ''), 10);
          if (e.target.checked) {
            if (testsCur >= mv - 5) { testsCur = mv; }
            else { e.target.checked = false; toast(t('testOrderErr')); return; }
          } else {
            if (testsCur >= mv) testsCur = mv - 5;
          }
          [5, 10, 15, 20, 25, 30].forEach(function (ms) {
            const box = document.getElementById('m-test-' + ms);
            if (box) box.checked = testsCur >= ms;
          });
        }
      });
      b.addEventListener('click', function (e) {
        ...
        if (a.getAttribute('data-action') === 'm-save') {
          ...
          const fsRaw = document.getElementById('m-fromstart').value;
          const feRaw = document.getElementById('m-fromend').value;
          const fsV = fsRaw === '' ? null : (parseInt(fsRaw, 10) || 0);
          const feV = feRaw === '' ? null : (parseInt(feRaw, 10) || 0);
          if (!isQaida && (fsV || 0) + (feV || 0) > 30) { toast(t('fromStartEndBad')); return; }
          const st2 = Object.assign({}, currentEdit, {
            ...
            fromStart: !isQaida ? fsV : null,
            fromEnd: !isQaida ? feV : null,
    ...
    const bodyFor = function (s) {
      ...
      out += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">' +
        ...
        '<div class="field"><label for="qe-lines-' + s.id + '">' + t('lines') + '</label>' +
          '<select id="qe-lines-' + s.id + '" data-qe="lines" data-sid="' + s.id + '">' + (track === 'qaida' ? linesOpts(st.lines) : rangeOpts(20, st.lines)) + '</select></div>' +
      '</div>';
      ...
      if (track !== 'qaida') {
        if (manzilIsTri) {
          out += '<div style="margin-top:8px' + (st.manzilDone ? '' : ';display:none') + '" id="qe-trim-' + s.id + '">' +
            '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">' +
            '<div class="field"><label for="qe-mzpara-' + s.id + '">' + t('manzilPara') + '</label>' +
              '<select id="qe-mzpara-' + s.id + '" data-qe="manzilPara" data-sid="' + s.id + '">' + rangeOpts(30, st.manzilPara) + '</select></div>' +
            '<div class="field"><label for="qe-mzsel-' + s.id + '">' + t('manzil') + '</label>' +
              '<select id="qe-mzsel-' + s.id + '" data-qe="manzil" data-sid="' + s.id + '">' + manzilOptsHtml(st.manzil) + '</select></div>' +
            '</div>' +
          '</div>';
        } else {
          out += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px' + (st.manzilDone ? '' : ';display:none') + '" id="qe-trim-' + s.id + '">' +
      ...
      const commentBox = '<div class="field" style="margin-top:8px">' +
        '<input id="qe-comment-' + s.id + '" data-qe="comment" data-sid="' + s.id + '" placeholder="' + t('commentHint') + '" value="' + esc(st.comment) + '">' +
      '</div>';
      const absentBox =
        '<div id="qe-absent-' + s.id + '"' + (st.present ? ' style="display:none"' : '') + '>' +
          '<div style="display:inline-flex;align-items:center;gap:8px;margin-top:10px">' +
            '<label class="switch"><input type="checkbox" id="qe-reason-' + s.id + '" data-qe="reason" data-sid="' + s.id + '"' + (st.reason ? ' checked' : '') + '><span class="track"></span></label>' +
            '<span class="lbl" style="font-size:.88rem">' + t('reason') + '</span>' +
          '</div>' +
        '</div>';
      return (
        '<div id="qe-body-' + s.id + '">' +
          '<div id="qe-detail-' + s.id + '"' + (st.present ? '' : ' style="display:none"') + '">' +
            out +
          '</div>' +
          absentBox +
          commentBox +
...
'<div class="meta">' + (trackOf(s) === 'qaida' ? t('lessonNo') + ' ' + num(s.para) : t('para') + ' ' + num(s.para) + ' · ' + t('page') + ' ' + num(s.currentPage || '—')) + '</div>' +
      ...
      shifts.forEach(function (sh) {
        ...
        out += '<div class="shift-group" data-shift="' + sh.key + '">' +
               '<div class="shift-header" data-action="toggle-shift" data-shift="' + sh.key + '" style="cursor:pointer"><span class="shift-arrow">' + (collapsedShifts[sh.key] ? '▸' : '▾') + '</span> 🕗 ' + sh.label + ' <span class="badge">' + grp.length + ' ' + t('shiftStudents') + '</span>' +
      ...
      if (unassigned.length) {
        out += '<div class="shift-group" data-shift="un">' +
               '<div class="shift-header" data-action="toggle-shift" data-shift="un" style="cursor:pointer"><span class="shift-arrow">' + (collapsedShifts['un'] ? '▸' : '▾') + '</span> — ' + t('shiftNone') + ' <span class="badge">' + unassigned.length + ' ' + t('shiftStudents') + '</span>' +
    ...
    function wireWidgets() {
      students.forEach(function (s) {
        const ids = ['qe-pages-' + s.id, 'qe-lines-' + s.id, 'qe-mpages-' + s.id, 'qe-mlines-' + s.id, 'qe-mzsel-' + s.id, 'qe-mzpara-' + s.id, 'qe-comment-' + s.id];
        ids.forEach(function (id) {
          const el = document.getElementById(id);
          if (el) {
            el.addEventListener('input', function () {
              if (id.indexOf('comment') > -1) state[s.id].comment = el.value;
              else if (id.indexOf('mzpara') > -1) state[s.id].manzilPara = parseInt(el.value, 10) || 0;
              ...
              else if (id.indexOf('qe-lines-') === 0) state[s.id].lines = parseFloat(el.value) || 0; /* qaida 0.5 */
    ...
    viewActions['qe-present'] = function (btn) {
      ...
      const det = document.getElementById('qe-detail-' + sid);
      if (det) det.style.display = state[sid].present ? '' : 'none';
    ...
    function saveStudentList(list, stay, okMsg) {
      ...
      for (const s of list) {
        ...
        if (chk.present && chk.manzilDone && trk === 'hifz' && !chk.manzilPara) {
          toast(t('manzilParaRequired') + ' — ' + nameClean(s.name));
          return;
        }
      }
      const jobs = list.map(function (s) {
        ...
        const rep = {
          ...
          manzilPara: st2.present && st2.manzilDone && manzilIsTri ? (st2.manzilPara > 0 ? st2.manzilPara : null) : null,
  ...
  function csvCell(v) {
    ...
    /* neutralize spreadsheet formula injection: a leading = + - @ (or tab/CR)
       in a name/comment would run as a formula on the principal's machine.
       The quote prefix forces plain text and stays invisible in Excel/Sheets. */
    if (/^[=+\-@\t\r]/.test(s)) s = "'" + s;
    return '"' + s.replace(/"/g, '""') + '"';
  }
  ...
  async function exportStudentReportCard(sid) {
    ...
    w.document.write('<!DOCTYPE html><html><head><title>' + nameDisplay(st.name) + ' — Report Card</title><style>' +
      ...
      '<div class="grid">' +
        ...
        (st.testsPassed > 0 ? '<div class="g"><b>' + st.testsPassed + '</b>Tests Passed</div>' : '') +
